<?php
	...
	class Texto
	{
		...
		public function registrarDado ($p_dados)
		{
			...
			for ($i = 0; $i < count($p_dados) - 1; $i++)
			{
				$str_dados = $str_dados.$p_dados[$i].$this->_divisor;
			}
			$str_dados = $str_dados.$p_dados[count($p_dados) - 1]."\n";
			var_dump($str_dados);
			fwrite($handle, $str_dados);
		}
		...
		public function removerLinha ($p_id)
		{
			$dados = $this->toArray();
			$novosDados = array();
			$id = 0;
			for ($i = 0; $i < count($dados); $i++)
			{
				if ( $i != $p_id )
				{
					$novosDados[$id] = $dados[$i];
					$novosDados[$id][0] = $id;
					$id++;
				}
			}
			$this->renovarDados($novosDados);
		}
	}